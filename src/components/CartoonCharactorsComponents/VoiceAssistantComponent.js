...
import {HarryPorterComponent} from "./HarryPorterComponent";
import {AlienComponent} from "./AlienComponent";
import {AkuAkuComponent} from "./AkuAkuComponent";
import {HorrorCartoonComponent} from "./HorrorCartoonComponent";
...
export default function VoiceAssistantComponent() {
    ...
    const assistantSearchTestForSpeak = useSelector((state) => state.assistantSearchTestForSpeak);
    ...
    const callFunctionToGetCommandsForTextSpeech = (text)=>{
        ...
        dispatch(actionToGetCommandsForTextSpeech(text,setLypSyncType,Capacitor.isNativePlatform(),speechFunction));
    }
    ...
    const getCommandWiseActions = (command) => {
        switch (command?.trim()?.toLowerCase()) {
            ...
            case 'okay':
            case 'no problem':
                callFunctionToGetCommandsForTextSpeech('Yes');
                resetTranscript();
                break;
            case 'hello':
            case 'hi':
    ...
    const callFunctionToOpenVoiceBot = () => {
        ...
        callFunctionToStartStopSpeech(false);
        dispatch(actionToOpenVoiceBot(setLypSyncType,Capacitor.isNativePlatform(),speechFunction));
    }
    ...
    return (
        <>
            ...
            <div style={{display:wakeupAssistant ? 'flex' : 'none'}} className={"character_voice_command_main_container"}>
                ...
                <div className={"floating_answer_question_main_div"}>
                    <div id={"floating_answer_question_inner_div"} className={"floating_answer_question_inner_div"}>
                        {assistantSearchTestForSpeak}
                    </div>
                </div>
                {
                    (userVoiceAssistantSelection === 'minion') ?
                        <MinionComponent lypSyncType={lypSyncType}/>
                    :
                    (userVoiceAssistantSelection === 'harry') ?
                        <HarryPorterComponent lypSyncType={lypSyncType}/>
                    :
                    (userVoiceAssistantSelection === 'alien') ?
                       <AlienComponent lypSyncType={lypSyncType}/>
                    :
                    (userVoiceAssistantSelection === 'akuaku') ?
                       <AkuAkuComponent lypSyncType={lypSyncType}/>
                    : (userVoiceAssistantSelection === 'horror') ?
                       <HorrorCartoonComponent lypSyncType={lypSyncType}/>
                    :''
                }
            </div>
        </>
