...
import {_generateUniqueIdForBlock, _userSchoolAvatarObject} from "../../helpers/CommonHelper";
import {IonAlert} from "@ionic/react";
import {useHistory} from "react-router-dom";
import {IonContent} from "@ionic/react";

export const AddNewTeacherFormComponent = ()=> {
    ...
    const [schoolClassWithSectionId, setSchoolClassWithSectionId] = useState(0);
    const [isPasswordAlreadyInUse, setIsPasswordAlreadyInUse] = useState(false);
    const [showSuccessAlert, setShowSuccessAlert] = useState(false);
    const [savingMode, setSavingMode] = useState(false);
    const dispatch = useDispatch();
    const allClassSectionData = useSelector((state) => state.allClassSectionData);
    ...
    const validateForm = () => {
        if(isPasswordAlreadyInUse){
            ...
            return false;
        }else if (!schoolClassWithSectionId) {
            return false;
        }else if (!avatar) {
            return false;
    ...
    const callFunctionToSubmit = async (e)=>{
        ...
        if(validateForm()){
            ...
            let payload = {
                ...
                password:password,
                mobile:mobile,
                syllabus_type:userInfo.syllabus_type_id,
                avatar:avatar,
                school_id:userInfo.id,
                school_class_with_section_id:schoolClassWithSectionId,
                role:3,
            }
                ...
                <form onSubmit={callFunctionToSubmit}>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            value={name}
                            type="name" className="form-control" aria-describedby="emailHelp" placeholder="Teacher name" required/>
                    </div>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            value={email}
                            type="email" className="form-control" aria-describedby="emailHelp" placeholder="Teacher email" required/>
                    </div>
                    <div className="form-group">
                        ...
                        <input
                            ...
                            value={mobile}
                            type="number" className="form-control" aria-describedby="emailHelp" placeholder="Teacher mobile number" required/>
                    </div>
                    <div className="form-group">
                        <label htmlFor="exampleInputEmail1">Class Section</label>
                        <select value={schoolClassWithSectionId} onChange={(e)=>setSchoolClassWithSectionId(Number(e.target.value))} className="form-control">
                            <option value={0}>Select class section</option>
                            {allClassSectionData?.map((classSection,key)=>(
                                <option key={key} value={classSection?.id}>{classSection.standard}{classSection?.section}</option>
                            ))}
                        </select>
                    </div>
                    <div className="form-group">
