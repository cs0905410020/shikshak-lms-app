...
export const actionToGetAllPincodeListDataQuery = ()=>{
    return `SELECT * FROM pincode order by id desc`;
}
export const actionToGetPinCodeDetailsDataQuery = (pinCode)=>{
    return `SELECT * FROM pincode where pincode = ${pinCode} order by id desc`;
}
export const actionToGetAllStateListQuery = ()=>{
    return `SELECT id,name as sate_name FROM state_list order by sate_name ASC`;
}