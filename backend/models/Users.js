 ...
 * @returns {{query: string}}
 */
export const actionToGetUsersListApiCall = () =>{
    return new Promise(function(resolve, reject) {
        const query = `SELECT u.*, r.role as role_name,c.name AS company_name from users u join role r on r.id=u.role
                                                                                           join company c on c.id=u.source where u.is_active = '1'`;
        pool.query(query, (error, results) => {
            if (error) {
                reject(error)
            }
