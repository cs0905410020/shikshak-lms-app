...
PaymentRouters.post(
    ...
    expressAsyncHandler(async (req, res) => {
        ...
        paymentGateway.orders.create(options, function(err, order) {
            if(order){
                let data ={
                    success: true,
                    order,
                ...
                res.status(500).json(CryptoJS.AES.encrypt(JSON.stringify(data), ENCRYPTION_KEY).toString());
            }
        });
    })
);
...
PaymentRouters.post(
    ...
    expressAsyncHandler(async (req, res) => {
        paymentGateway.payments.refund(options, function(err, order) {
            if(order){
                res.status(200).json({
                    success: true,
                    order,
