...
import {paymentGateway} from "../server.js";
...
import {refundRazorpayPayment} from "../helper/PaymentHelper.js";
productsRouter.post(
    '/actionToGetProductsApiCall',
...
productsRouter.post(
    ...
    expressAsyncHandler(async (req, res) => {
        actionGetOrderDetailsData(req?.body).then( async (data) => {
            if(data) {
                // res.status(200).send(data[0]?.orderData);
                if (data[0]?.orderData?.paymentStatus === "1" && data[0]?.orderData?.payment_gateway_name === "phonepay") {
                    let options = {
                        "speed": "normal",
                        "notes": {
                            "order_number": data[0]?.orderData?.order_number
                        }
                    };
                    paymentGateway.payments.refund(data[0]?.orderData?.payment_gateway_payment_id, options, function (err, order) {
                        if (order) {
                            res.status(200).json({
                                success: true,
                                order,
                ...
                if (data[0]?.orderData?.paymentStatus === "1" && data[0]?.orderData?.payment_gateway_name === "razorpay") {
                    await refundRazorpayPayment(data[0]?.orderData).then( async (responseRefundPayment)=>{
                        if(responseRefundPayment?.id){
                            await savePaymentGatewayTransactionAfterRefund(data[0]?.orderData?.id,data[0]?.orderData?.payment_gateway_id,responseRefundPayment?.payment_id,'refund','2',responseRefundPayment).then(async (respo)=> {
